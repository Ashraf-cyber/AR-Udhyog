...
class SoldOrder extends StatefulWidget {
  const SoldOrder({Key? key, required this.title}) : super(key: key);
  final String title;

  @override
  State<SoldOrder> createState() => _MyHomePageState();
}

class _MyHomePageState extends State<SoldOrder> {
  ...
  Future<void> uploadInventoryInfo() async {
    ...
    if ( quantity.text.isNotEmpty &&
      ...
      var request = http.MultipartRequest(
          'POST',
          Uri.parse(
              "https://mypottery.000webhostapp.com/MyPottery/SellProduct.php"));

      // request.files
      //     .add(await http.MultipartFile.fromPath('image', uploadimage!.path)
      // );

      request.fields["CUSTOMER_NAME"] = customer_name!;
  ...
  void getCustomer() async {
    ...
    if (response.statusCode == 200) {
      data = response.body; //store response as string
      setState(() {
        result2 = jsonDecode(data!)['data']; //get all the data from json string hospital length
        for(int x=0;x<result2.length;x++)
        {
          customer.add(result2[x]['OWNER_NAME']);
        }
        print(result2); // just printed length of data
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Product Sold To"),
      ),
      body: Center(
                      ...
                      child: Card(
                        ...
                        child: Padding(
                          ...
                          child:  DropdownButton<String>(
                            hint: const Text("Select Customer",style: TextStyle(color: Colors.white),),
                            isExpanded: true,
