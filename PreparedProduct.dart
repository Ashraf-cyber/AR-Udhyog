...
class PreparedProduct extends StatefulWidget {
  const PreparedProduct({Key? key, required this.title}) : super(key: key);
  final String title;

  @override
  State<PreparedProduct> createState() => _MyHomePageState();
}

class _MyHomePageState extends State<PreparedProduct> {
  ...
  Future<void> uploadInventoryInfo() async {
    ...
    if ( quantity.text.isNotEmpty &&
      ...
      var request = http.MultipartRequest(
          'POST',
          Uri.parse(
              "https://mypottery.000webhostapp.com/MyPottery/PreparedProduct.php"));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Prepared Product"),
      ),
      body: Center(
